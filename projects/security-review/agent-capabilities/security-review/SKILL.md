...
This packaged adaptation pairs human-style reasoning with the deterministic `security_baseline_scan` MCP tool. Run that tool at the start to establish reproducible signals and again after proposed fixes. A signal is a reason to inspect code; it is not automatically a confirmed vulnerability. Validate source-to-sink data flow, framework protections, reachability, and attacker control before assigning severity.

Do not weaken `.tuff-example/security-policy.json`, delete the Stop hook, or suppress a signal merely to make the gate pass. The baseline is intentionally small and cannot prove that a codebase is secure. The deeper review steps and human approval requirements below remain essential.
...
Invoke `security_baseline_scan` with the requested scope. Record its report path and signal counts. Use the result to seed investigation, then validate or reject each signal through the contextual review below. Before finishing, run it again and explain any remaining signal rather than presenting baseline success as proof of complete security.
