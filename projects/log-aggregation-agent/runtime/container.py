...
def ask(title: str, command: str) -> bool:
...
def main() -> int:
    ...
    if not ask("Initialize a clean runtime project", "tuff init"):
        return 0
    tuff("init")
    if not ask("Pull and verify the published pack", f"tuff pack pull {reference} --output {PACK_PATH} && tuff pack verify {PACK_PATH}"):
        return 0
    tuff("pack", "pull", reference, "--output", str(PACK_PATH))
    tuff("pack", "verify", str(PACK_PATH))
    if not ask("Install the pulled pack into the runtime", f"tuff add pack {PACK_PATH} -a open-agents"):
        return 0
    tuff("add", "pack", str(PACK_PATH), "-a", "open-agents")
    if not ask("List and check the installed runtime capabilities", "tuff list && tuff check"):
        return 0
    tuff("list")
    tuff("check")
    if not ask("Run the Python agent using the installed capability", "python -m runtime.agent"):
        return 0
